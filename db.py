    ...
    try:
        ...
        for row in rows:
            ...
            result.append({
                ...
                "pricePerHour": float(row.get("PricePerHour") or row.get("pricePerHour")) if row.get("PricePerHour") or row.get("pricePerHour") is not None else 0.0,
...
def get_all_memberships():
    ...
    try:
        cursor = conn.cursor(as_dict=True)
        cursor.execute(SP_GET_ALL_MEMBERSHIPS)
        rows = cursor.fetchall()
        for row in rows:
